...
#### Gemini and Antigravity directories

`~/.gemini/skills/` is always the Gemini CLI Skill directory. When an Antigravity
(agy) environment is detected and Gemini Skill sync is enabled, CC Switch also
manages `~/.gemini/config/skills/` as a synchronized projection: enabled Skills
are written or updated there, and disabled managed Skills are removed.

`~/.gemini/antigravity*/skills/` directories are client-private locations. CC
Switch scans them for discovery and import only; it never writes to or deletes
their contents during Gemini Skill sync.
